...
from .directive import FlexTreeNode
...
def note_toctree(env: BuildEnvironment, docname: str, toctreenode: FlexTreeNode) -> None:
...
def global_toctree_for_doc(
    ...
    resolved = (
        ...
        for toctree_node in env.master_doctree.findall(FlexTreeNode)
    )
    toctrees = [
...
def _resolve_toctree(
    env: BuildEnvironment, docname: str, builder: Builder, toctree: FlexTreeNode, *,
    ...
    for (title, ref) in toctreenode['entries']:
        ...
        if titles_only:
            ...
            for top_level in children:
                ...
                if len(top_level) > 1:
                    if subtrees := list(top_level.findall(FlexTreeNode)):
                        top_level[1][:] = subtrees  # type: ignore[index]
                    else:
                        top_level.pop(1)
        # resolve all sub-toctrees
        for sub_toc_node in list(toc.findall(FlexTreeNode)):
...
def _toctree_copy(node: ET, depth: int, maxdepth: int, collapse: bool, tags: Tags) -> ET:
    ...
    for subnode in node.children:
        ...
        elif isinstance(subnode, FlexTreeNode):
...
class TocTree:
    ...
    def note(self, docname: str, toctreenode: FlexTreeNode) -> None:
        note_toctree(self.env, docname, toctreenode)

    def resolve(self, docname: str, builder: Builder, toctree: FlexTreeNode,
                prune: bool = True, maxdepth: int = 0, titles_only: bool = False,
